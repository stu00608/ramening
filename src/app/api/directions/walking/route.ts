...
// 徒步導航參數驗證 schema
const WalkingDirectionsSchema = z
  .object({
...
export async function GET(request: NextRequest) {
  try {
    ...
    // 準備驗證資料
    const validationData: Record<string, string | number> = {};
    ...
    if (destinationPlaceId)
      validationData.destinationPlaceId = destinationPlaceId;
    if (destinationLat)
      validationData.destinationLat = Number.parseFloat(destinationLat);
    if (destinationLng)
      validationData.destinationLng = Number.parseFloat(destinationLng);
    ...
    if (data.status !== "OK") {
      ...
      if (
        ...
        if (
          ...
          !validatedData.destinationLat ||
          !validatedData.destinationLng
        ) {
        ...
        const deltaLatRad =
          ((validatedData.destinationLat - validatedData.originLat) * Math.PI) /
          180;
        const deltaLngRad =
          ((validatedData.destinationLng - validatedData.originLng) * Math.PI) /
          180;
      ...
      return NextResponse.json(
        {
          error: "Google Directions API 請求失敗",
    ...
    if (!data.routes || data.routes.length === 0) {
      return NextResponse.json({ error: "找不到徒步路線" }, { status: 404 });
    }
